...
Este guía tiene como objetivo proporcionarle una comprensión clara de los conceptos fundamentales necesarios para crear, configurar y gestionar sus primeras instancias de OVHcloud Public Cloud Compute. Aprenderá cómo funcionan las instancias, cómo elegir el tipo correcto de instancia y cómo los elementos clave, como las imágenes, las zonas de disponibilidad, la red, la seguridad y las copias de seguridad, se integran dentro del ecosistema de OVHcloud.

## ¿Qué es una instancia (Máquina Virtual)?

Una instancia, o Máquina Virtual (VM), es un servidor completamente aislado que se ejecuta sobre la infraestructura física compartida de OVHcloud. Funciona como un servidor tradicional, pero ofrece la flexibilidad y escalabilidad de la nube. Usted elige el sistema operativo, define los recursos de CPU, RAM y Almacenamiento, y despliega sus aplicaciones, sitios web o entornos de desarrollo.

Las instancias de Public Cloud Compute ofrecen:

- Creación a demanda y escalado flexible – Escalar recursos según sus necesidades.
...
| Optimizada CPU     | Alta performance del procesador  | Ideal para aplicaciones intensivas de cálculo, tareas de procesamiento paralelo, pipelines CI/CD o microservicios que requieren una alta frecuencia de CPU.                                     |
| Optimizada Memoria | Alta capacidad de memoria        | Diseñada para el análisis de datos, cargas de trabajo de big data y caché de bases de datos. Presenta altas relaciones memoria/CPU y IOPS acelerados. Los núcleos virtuales están a 2 GHz o más.       |
| Optimizada Almacenamiento | Alta performance IOPS           | Equipada con almacenamiento NVMe para E/S de disco ultrarrápidas, ideal para bases de datos y aplicaciones de big data.                                                                     |
| GPU                | Gráficos acelerados por hardware | Proporciona una excepcional performance de cálculo paralelo, hasta 1000 veces más rápida que la CPU para ciertas cargas de trabajo. Adecuada para IA, aprendizaje profundo y renderizado 3D.               |
| Descubrimiento     | Recursos compartidos, económica  | Instancias de entrada con recursos compartidos, ofreciendo una performance estable a un precio asequible. Ideal para entornos de prueba, formación o proyectos de demostración. | 
...
Las instancias de Public Cloud de OVHcloud se despliegan en [varios centros de datos a lo largo del mundo](&/links/infrareg), garantizando una alta disponibilidad y proximidad con sus usuarios. Ejemplos de regiones:
...
Al crear una instancia, selecciona una imagen que incluye el sistema operativo y, opcionalmente, aplicaciones preinstaladas. OVHcloud ofrece una variedad de imágenes para satisfacer necesidades diversas:
...
**Ciclo de vida y soporte :** OVHcloud actualiza regularmente el catálogo de imágenes. Siempre consulte las anuncios sobre el ciclo de vida y el final del soporte para asegurarse de que sus imágenes siguen seguras y soportadas. Vea [aquí](&/pages/public_cloud/Compute/image-life-cycle).
...
La autenticación SSH garantiza un acceso cifrado y fiable a sus servidores.
...
| -----------------------| --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Red pública          | Las instancias están conectadas a Internet a través de una dirección IP pública.                         | Alojamiento de sitios web, APIs o proporcionar acceso remoto a sus servidores.                  |
...
Para más detalles sobre la configuración de las redes de Public Cloud, consulte la guía oficial [OVHcloud sobre redes](&/pages/public_cloud/public_cloud_network_services/concepts-01-public-cloud-networking-concepts).
...
Una vez que domine los conceptos fundamentales del Public Cloud Compute de OVHcloud, puede explorar operaciones y tareas de gestión más avanzadas.
...
- [Añadir créditos cloud](&/pages/account_and_service_management/managing_billing_payments_and_services/add_cloud_credit_to_project)
