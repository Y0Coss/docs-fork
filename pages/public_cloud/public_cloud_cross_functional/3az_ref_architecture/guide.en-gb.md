...
| Block storage Classic Multi-Zone | | <center>X</center> | Block Storage Classic is a regional service using distributed erasure coding across several AZs. Off-site replication is recommended to protect against regional failure. | Block Storage data remains available without impact or downtime, provided the conditions of the multi-attached resilient architecture are met. (cf: [This guide](/pages/compute/classic_block_multi_az_limitations)). In the event of a major incident, chunks will be recreated as soon as the AZ is restored. |
...
- The Block Storage service is regional, shared between the AZs and simultaneously attached to both Instance 1 on AZ-a and Instance 2 on AZ-b (cf: [This guide](/pages/compute/classic_block_multi_az_limitations))
