...
Ce guide a pour objectif de former et d’accompagner les clients dans la migration de leur infrastructure vers le Public Cloud en utilisant l’Infrastructure en tant que Service (IaaS). Il propose une méthodologie détaillée étape par étape pour inventorier les workloads existantes, préparer une landing zone sécurisée et évolutive, et exploiter des outils tels que Terraform/OpenTofu, Ansible, rclone et rsync afin d’automatiser le déploiement et le transfert des données. Le guide présente également les bonnes pratiques pour sélectionner les types d’instances OVHcloud compatibles, configurer les réseaux et garantir la continuité des services pendant la migration. En suivant ce guide, les utilisateurs pourront construire une infrastructure robuste, optimisée en coûts et native cloud, parfaitement adaptée à l’écosystème IaaS d’OVHcloud.
...
Avant de détailler les étapes spécifiques que je recommande, le schéma ci-dessous offre une perspective générale des principales phases d'une migration vers OVHcloud Public Cloud.

![IAAS migration schema](images/schema_migration.png){.thumbnail}

### 1. Faire l'inventaire de vos VMs sources, du stockage et de la configuration réseau
...
- **Machines virtuelles (VMs):** Listez toutes vos VM en regroupant celles appartenant à une même application (par exemple, un serveur web, un serveur applicatif et une base de données). Cette méthode facilite la migration application par application, afin de minimiser les interruptions de service.
- **Stockage:** Identifiez vos volumes de stockage en bloc ainsi que vos buckets de stockage objet.
- **Ressources réseaux:** Cartographiez l’ensemble de votre topologie réseau, incluant les réseaux de couche 2 (L2) et couche 3 (L3), les routeurs, les load balancers et les firewalls.
...
Utilisez les données d’inventaire de vos VM, stockage, réseau et dispositifs de sécurité pour affiner le zonage et la segmentation dans le cloud. Corriger les problèmes à ce stade permet d’obtenir une Landing Zone sécurisée, conforme, facile à gérer et optimisée en coûts.
...
Bien que le Public Cloud d’OVHcloud repose sur OpenStack, il propose également des services propriétaires comme Kubernetes managé et Bases de données managées. Pour cette raison, vous devrez utiliser des providers OpenTofu ou Terraform spécifiques, adaptés à l’infrastructure OVHcloud.

Vous pouvez créer vos stacks Terraform/OpenTofu en utilisant les providers pour [OpenStack](https://registry.terraform.io/providers/terraform-provider-openstack/openstack/latest/docs){.external}, OVHcloud, ou des services compatibles S3, selon les ressources que vous souhaitez gérer.

[Voici un lien](/pages/manage_and_operate/terraform/terraform-at-ovhcloud) avec des exemples pour vous aider à démarrer la création de vos configurations Terraform/OpenTofu pour votre landing zone et votre infrastructure cloud.
...
Avant de déployer vos workloads dans le Public Cloud, vous devez choisir les types d’instances (« flavours ») qui correspondent le mieux à vos machines virtuelles (VM) existantes. Cela garantit que vos applications migrées fonctionneront comme prévu.

Pour ce faire :

- Analysez les configurations de vos VM sources (CPU, RAM, stockage, etc.)
...
Une fois vos choix effectués, notez les identifiants techniques des instances (ex. : b3-64, c3-128) afin de pouvoir les utiliser directement dans votre configuration Terraform/OpenTofu.

N’oubliez pas de faire de même pour :

- Les gateways (ex. : tailles M, L, XL)
...
La méthode Lift and Shift vous permet de migrer vos machines virtuelles (VM) existantes sans modifier significativement leur configuration ou architecture. C’est une façon rapide de déplacer vos charges de travail vers le Public Cloud OVHcloud avec un minimum de réingénierie.

Pour commencer, exportez vos VM sources depuis votre infrastructure actuelle. OVHcloud supporte plusieurs formats d’images, notamment : ami, ari, aki, vhd, vmdk, raw, qcow2, vhdx, vdi, iso, et ploop.

Une fois exportées, vous devez téléverser l’image de VM dans votre projet OVHcloud. Pour cela, assurez-vous d’avoir un environnement Python local prêt et installez le client OpenStack CLI ([voir guide d’installation](https://docs.openstack.org/newton/user-guide/common/cli-install-openstack-command-line-clients.html){.external}). Vous aurez également besoin de vos identifiants API pour vous authentifier.

Après avoir téléversé l’image, vous pouvez déployer une nouvelle instance à partir de celle-ci via le tableau de bord OVHcloud, la CLI OpenStack ou via Terraform/OpenTofu. Retrouvez les instructions détaillées pour importer une image de VM dans [ce guide](/pages/public_cloud/compute/upload_own_image).
...
Si vous migrez plusieurs VM, envisagez d’automatiser ce processus avec des scripts ou des outils d’infrastructure as code. Vous pouvez aussi contacter notre [équipe Professional Services](https://www.ovhcloud.com/fr/professional-services/){.external} pour obtenir de l’aide ou déléguer entièrement la migration.

#### 6b. Effectuer une replatforming avec des déploiements propres

La méthode de replatforming consiste à créer de nouvelles machines virtuelles dans votre environnement OVHcloud Public Cloud et à réinstaller vos applications à partir de zéro. Cette approche est idéale pour un déploiement propre ou pour tirer parti des outils modernes d’automatisation et d’infrastructure.

Vous pouvez provisionner les nouvelles instances via vos stacks Terraform/OpenTofu ou directement depuis le tableau de bord OVHcloud, en choisissant les régions, zones de disponibilité et types d’instances appropriés.
...
### 7. Migrer le stockage en blocs avec rsync ( ou un outil similaire )
...
Si vous avez besoin d’aide pour configurer ou automatiser ce processus sur plusieurs instances, vous pouvez contacter [l’équipe Professional Services](https://www.ovhcloud.com/fr/professional-services/){.external} d’OVHcloud.

### 8. Migrer le stockage objet avec rclone ( ou un outil similaire )
...
Cette méthode assure un transfert fiable et reprenable, même pour de gros volumes de données. Suivez ce [guide](/pages/storage_and_backup/object_storage/s3_rclone) pour connaître toutes les étapes nécessaires.
...
Si votre configuration inclut des fonctionnalités avancées (versioning, règles de cycle de vie, etc.) ou si vous souhaitez automatiser le processus, [l’équipe Professional Services](https://www.ovhcloud.com/en/professional-services/){.external} d’OVHcloud peut vous accompagner avec une approche personnalisée.

### 9. Reconnecter les endpoints et valider les applications
...
### 10. Mettre à jour les enregistrements DNS
...
### 11. Décommissionner l'infrastructure existante
...
Pour mieux gérer et réduire vos dépenses cloud, appliquez les bonnes pratiques FinOps. OVHcloud propose plusieurs solutions d’optimisation des coûts, dont des [Saving Plans](https://www.ovhcloud.com/en/public-cloud/savings-plan){.external} adaptés à vos habitudes de consommation.

Consultez [ce guide](/pages/public_cloud/public_cloud_cross_functional/savings_plans) pour découvrir comment piloter efficacement vos coûts et optimiser vos usages dans le cloud.
