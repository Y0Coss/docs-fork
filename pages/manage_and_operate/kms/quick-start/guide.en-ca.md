...
You can order a KMS from the [OVHcloud Control Panel](/links/manager) by going to `Identity, Security & Operations`{.action} and `Key Management Service`{.action} then the `Order a KMS`{.action} button.
