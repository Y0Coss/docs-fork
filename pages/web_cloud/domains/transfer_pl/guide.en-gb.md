...
Once the order has been confirmed, you will receive a free purchase order. The transfer process will only start once you have confirmed this free purchase order. Once this is done, you can track the transfer progress via the [OVHcloud Control Panel](/links/manager){.external}. To track the progress, log in to the [OVHcloud Control Panel](/links/manager), then go to the `Web Cloud`{.action} section.. Click on `Ongoing operations`{.action}.
...
Once the transfer procedure is complete, you can manage your domain name from the [OVHcloud Control Panel](/links/manager){.external}. To do this, select `Web Cloud`{.action}, click `Domain names`{.action}, then click on the domain name concerned.
