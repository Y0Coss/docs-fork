...
Haben Sie eine E-Mail von OVHcloud erhalten, die besagt, dass für Ihre Domain eine Aktion erforderlich ist? Zeigt diese E-Mail an, dass die Kontaktdaten (Kontaktinformationen) Ihres Domaininhabers korrigiert oder vervollständigt werden müssen?

**Erfahren Sie, wie Sie die Kontaktdaten eines Domaininhabers nach Empfang einer Warn-E-Mail von OVHcloud korrigieren oder vervollständigen können.**

### Warum habe ich eine Verifikations-E-Mail von OVHcloud erhalten?
...
Nach dem Kauf erhält der Domaininhaber eine E-Mail von OVHcloud, um seine E-Mail-Adresse zu validieren und zu bestätigen, dass sie erreichbar ist.
...
Falls diese Methode nicht funktioniert, kann der Inhaber seine E-Mail-Adresse manuell mit dem Einmalcode, der in der E-Mail bereitgestellt wird, validieren, und auf `E-Mail-Validierungsformular`{.action} klicken.
...
Klicken Sie auf die Registerkarten unten, um jeden der **5** Schritte anzuzeigen.
...
Falls die mit Ihrer Domain verknüpfte E-Mail-Adresse falsch oder nicht zugänglich ist (z. B. aufgrund einer Tippfehler), sollten Sie [OVHcloud Support kontaktieren](/links/support).

Beachten Sie, dass der Domaininhaber erneut seine Identität nachweisen muss, indem er die erforderlichen Dokumente entsprechend seiner Art (Privatperson, Firma, Verein usw.) bereitstellt.

> [!primary]
>
> Die unten aufgeführte Dokumentenliste ist nicht vollständig. Andere Dokumente können je nach Ihrem Fall oder den Anforderungen der zuständigen Registrierstelle angefordert werden.
...
#### Spezialfall - Änderung der Inhaberkontaktdaten durch den Administrator ohne Unterstützung (Beispiel: .fr)

Für bestimmte Erweiterungen (z. B.: .fr, .it, .es) kann der Domainadministrator direkt in sein [OVHcloud Kundencenter](/links/manager) gehen und die E-Mail-Adresse des Inhaberkontakts gemäß den in Schritt 2 dieser Seite beschriebenen Schritten ändern.

### 4 - Operation zur Verifikation der Kontaktdaten des Domaininhabers erneut starten

> [!warning]
>
> Falls die Korrekturen im ersten Teil dieses Leitfadens den Start eines Wechsels des Inhabers für die Domain erfordern, vollenden Sie den Wechsel des Inhabers **vor** der Fortsetzung der in diesem Leitfaden beschriebenen Aktionen.
>

Klicken Sie auf die Registerkarten unten, um jeden der **4** Schritte anzuzeigen.
...
Falls dies nicht der Fall ist, ist eine Korrektur der Kontaktdaten des Domaininhabers erforderlich. In diesem Fall befolgen Sie bitte diesen Leitfaden erneut, um das Problem zu beheben.
