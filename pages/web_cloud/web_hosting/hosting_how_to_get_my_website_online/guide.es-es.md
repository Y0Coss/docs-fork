...
Si ya dispone de una base de datos (incluida con un [plan de hosting de OVHcloud](/links/web/hosting){.external}, una solución [Cloud Databases](/links/web/databases){.external}), necesitará el nombre de usuario y la contraseña, el nombre de la base de datos y la dirección del servidor. Una vez disponga de estos datos, vaya al siguiente paso.
