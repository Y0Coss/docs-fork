...
- Accès à l'[Espace client OVHcloud](/links/manager) ou à la [CLI Openstack](/pages/public_cloud/public_cloud_cross_functional/loading_openstack_environment_variables)
...
Suivez la procédure décrite dans [Étape 1 : Activer et gérer un vRack](/pages/public_cloud/public_cloud_network_services/getting-started-07-creating-vrack)
...
Suivez la procédure décrite dans [Étape 2 : Créer un réseau privé dans le vRack](/pages/public_cloud/public_cloud_network_services/getting-started-07-creating-vrack)

### Étape 3 — Démarrer l'instance sur le réseau privé

Déployez votre instance Public Cloud dans le réseau privé que vous avez créé. Assurez-vous qu'elle est connectée au sous-réseau correct pour permettre une communication sécurisée via le vRack.

Suivez la procédure décrite dans [Étape 3 : Intégrer une instance au vRack](/pages/public_cloud/public_cloud_network_services/getting-started-07-creating-vrack)

![instance creation on the Private Network](images/instance-in-private-net.png){.thumbnail}

Assurez-vous que l'instance est attribuée une adresse IP dans le sous-réseau sélectionné (par exemple, `10.235.0.15`).

Les deux méthodes atteignent le même objectif : connecter en toute sécurité votre service EFS à votre vRack et à vos instances Public Cloud.
...
/// details | a. Créer via les services vRack
...
1. Dans l'[Espace client OVHcloud](/links/manager), accédez à la section `Network`{.action}, aller dans `vRack Services`{.action}. 

2. Cliquez ensuite sur `Activer vRack Services`{.action}.

3. Sélectionnez la même région que votre vRack, votre instance Public Cloud et votre service EFS. <span id="step4-a-region-selection"></span>
...
5. Localisez votre nouvelle ressource de service vRack dans la liste et cliquez-y.

![vrack list](images/vrack_list.png){.thumbnail}

6. Modifiez le service vRack avec la configuration suivante :

- Private Network: Sélectionnez le même vRack utilisé pour votre projet Public Cloud.
...
> [!primary] 
> 
> Assurez-vous que le CIDR correspond à votre Private network Public Cloud.
>

1. CIDR du sous-réseau: e.g., `10.235.0.0/24`
2. Plage d'adresses du service: e.g., `10.235.0.240/28`
    - Sous-ensemble réservé du sous-réseau privé pour attribuer des adresses IP aux services EFS gérés dans le vRack.
3. VLAN: Utilisez le même numéro VLAN que votre réseau privé Public Cloud (voir [Step 2](#step2)).
4. Créer
...
/// details | b. Créer via la section Enterprise File Storage

1. Dans l'[Espace client OVHcloud](/links/manager), accédez à la section `Bare Metal Cloud`{.action}. Allez à `Stockage et sauvegarde`{.action}, `Enterprise File Storage`{.action}, puis sélectionnez votre service EFS.

2. Sous Configuration réseau, cliquez sur `Configurer les paramètres réseaux`{.action}.

![efs - configure network settings](images/efs_configure_network.png){.thumbnail}

3. Sélectionnez votre vRack

4. Si aucun service vRack n’a été créé, activez les services vRack et suivez [Étape 4a-3. Créer via les services vRack](step4-a-region-selection)
...
Dans l'onglet Contrôle d'accès de votre volume EFS :

- Ajoutez les adresses IP ou plages CIDR autorisées à monter le volume :

  - Pour autoriser une instance unique : `10.235.0.15`
  - Pour autoriser toutes les instances du sous-réseau : `10.235.0.0/24`

- Définissez le niveau d'accès souhaité : Lecture/Écriture ou Lecture seule.
...
### Étape 8 — (Facultatif) Activer le montage automatique au démarrage

Pour garantir que votre volume EFS est monté automatiquement au démarrage, ajoutez l'entrée suivante dans le fichier `/etc/fstab` :
...
- L'instance peut accéder au volume EFS via <IP DE VOTRE SERVICE EFS>.
