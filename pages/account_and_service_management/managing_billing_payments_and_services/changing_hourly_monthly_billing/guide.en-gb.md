...
When you create a Public Cloud instance, you can choose to be billed at either an hourly or monthly rate (except for third-generation Public Cloud instances, which are billed hourly by default and do not offer the monthly option during instance creation). Hourly-rate instances are billed on a pay-as-you-go basis, i.e. at the end of each month, users are billed for every commenced hour per actual resources used.<br>
