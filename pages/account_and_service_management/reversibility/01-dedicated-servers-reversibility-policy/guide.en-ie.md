...
|Feature|Description|Available formats|Migration model|Available documentation|
|---|-----|---|-----|-----|
|Load Balancing|Load Balancers are network devices that distribute requests across services and datacenters to ensure that there is no overload.|N/A|**Inbound migration**: Purchase and activation through [OVHcloud Control Panel](/links/manager).<br><br>**Outbound migration**: Order and set up a load balancer with the new provider.|**Inbound migration**: [OVHcloud Load Balancers](/links/network/load-balancer)<br><br>**Outbound migration**: N/A|
